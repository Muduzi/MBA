...
def service_income_per_group_this_month(buss):
    ...
    cache.set(str(buss) + 'services_m', services, 300)
    cache.set(str(buss) + 'packages_m', packages, 300)
    cache.set(str(buss) + 'customers_m', customers, 300)
...
def service_income_per_group_this_year(buss):
    ...
    cache.set(str(buss) + 'services_y', services, 300)
    cache.set(str(buss) + 'packages_y', packages, 300)
    cache.set(str(buss) + 'customers_y', customers, 300)
...
def service_monthly_records_this_year(buss):
    ...
    for month in range(0, 12):
        ...
        income_this_year[start.month] = {}
        income_this_year[start.month]['Amount'] = amount
        income_this_year[start.month]['Cash'] = cash
        income_this_year[start.month]['Credit'] = credit
    ...
    cache.set(str(buss) + 'income_this_year', income_this_year, 300)
...
def service_income_dash(request):
    try:
        ...
        try:
            general_content = ServiceGeneralContent.objects.get(Business__id=buss, Cashier=user_object)
        ...
        try:
            sales_this_year = ServiceAnnualContent.objects.get(Business__id=buss, Cashier=user_object)
        ...
        try:
            sales_this_month = ServiceMonthlyContent.objects.get(Business__id=buss, Cashier=user_object)
        ...
        services_daily_records_this_month.delay(buss)
        ...
        if not total_m and cash_m and credit_m and income_this_month:
            total_m, cash_m, credit_m, income_this_month = services_daily_records_this_month(buss)

        service_income_per_group_this_month.delay(buss)
        categories_m = cache.get(str(buss) + 'categories_m')
        services_m = cache.get(str(buss) + 'services_m')
        packages_m = cache.get(str(buss) + 'packages_m')
        customers_m = cache.get(str(buss) + 'customers_m')

        if not categories_m and services_m and packages_m:
            categories_m, services_m, packages_m, customers_m = service_income_per_group_this_month(buss)

        service_monthly_records_this_year.delay(buss)
        ...
        if not total_y and cash_y and credit_y and income_this_year:
            total_y, cash_y, credit_y, income_this_year = service_monthly_records_this_year(buss)

        service_income_per_group_this_year.delay(buss)
        categories_y = cache.get(str(buss) + 'categories_y')
        services_y = cache.get(str(buss) + 'services_y')
        packages_y = cache.get(str(buss) + 'packages_y ')
        customers_y = cache.get(str(buss) + 'customers_y')

        if not categories_m and services_m and packages_m:
            categories_y, services_y, packages_y, customers_y = service_income_per_group_this_year(buss)
